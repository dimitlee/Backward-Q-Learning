...
def run_episode(tempr):
    ...
    while not done:
        ...
        action2 = np.argmax(Q[current_state])
        eta =np.random.random_sample()
        temp0 = (Q[current_state][action1] - Q[current_state][action2])/tempr
        ...
        atuple = (current_state, action, reward, new_state) 
        memo.append(atuple)
    ...
    while(iteration>0):
        ...
        b_reward = btuple[2]
        b_next = btuple[3]
        b_act =  update_q(b_state, b_action, b_reward,b_next, alpha2, tempr)
        iteration = iteration - 1
    return duration
...
if __name__ == '__main__':
    durations = collections.deque(maxlen=3000)
