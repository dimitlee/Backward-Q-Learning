...
def update_q(state_old, action, reward, state_new, alpha, tempe):
    ...
    return next_action
def update_qq(state_old, action, reward, state_new, alpha, tempe):
...
def run_episode(tempr):
    ...
    while not done:
        ...
        next_ac = update_q(current_state, action, reward, new_state, alpha, tempr)
    ...
    while(iteration>0):
        ...
        b_reward = btuple[2]
        b_next = btuple[3]
        update_qq(b_state, b_action, b_reward,b_next, alpha2, tempr)
        iteration = iteration - 1
    return duration
