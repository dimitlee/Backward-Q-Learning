...
def qlearning(num_episodes = 500, gamma_discount = 0.95, alpha = 0.5, epsilon = 0.1):
    """
    ...
    for episode in range(0, num_episodes):
        ...
        while(game_end == False):
            ...
            # choose action1 arbitrarily
            action1 =  np.random.choice(4)
            # choose action using epsilon-greedy policy
            action2 = boltz_policy(state, q_table, temper)
            # random value btw 0 and 1
            eta =np.random.random_sample()
            # check
            temp0 = (q_table[action1, state] - q_table[action2, state])/temper
            temp1 = math.exp(temp0)  
            if eta<temp1:
                action = action1
            else:
                action = action2
            ...
            reward_cum += reward 
            # store in M
            atuple = (state, action, reward, next_state) 
            memo.append(atuple)
        ...
        while(iteration>0):
            ...
            q_table = update_qTable(q_table, b_state, b_action, b_reward,b_maximum_state_value, gamma_discount, alpha)
            iteration = iteration - 1
        if(gcount == 300):
            print("Agent trained with SA-Q-backward after 300 goal reachings")
...
# get the state from agent's position
                ...
                while(game_end == False):
                    ...
                    action1 =  np.random.choice(4)
                    # choose action using epsilon-greedy policy
                    action2 = np.argmax(q_table[:, state])
                    # random value btw 0 and 1
                    eta =np.random.random_sample()
                    # check
                    temp0 = (q_table[action1, state] - q_table[action2, state])/temper
