...
def qlearning(num_episodes = 500, gamma_discount = 0.95, alpha_init = 0.9, epsilon = 0.1):
    """
    ...
    for episode in range(0, num_episodes):
        ...
        while(game_end == False):
            ...
            reward_cum += reward
            temp = th_n - k*iteration
            temp2 = th_p + k*iteration 
            if temp < max_next_state_value and temp2 > max_next_state_value :
                ...
                temp3 = reward + gamma*max_next_state_value - q_table[action, state]
                alpha = np.tanh(beta*abs(temp3)/iteration)
            # update temperature
            temper = c/iteration
            # update q_table
            update_q_value = q_table[action, state] + alpha * temp3
            q_table[action, state] = update_q_value
            # update the state
            state = next_state
...
def sarsa(num_episodes = 500, gamma_discount = 0.9, alpha = 0.5, epsilon = 0.1):
    ...
    for episode in range(0, num_episodes):
        ...
        state, _ = get_state(agent, q_table)
        action = boltz_policy(state, q_table, temperature)
        while(game_end == False):
            # move agent to the next state
            agent = move_agent(agent, action)
            env = visited_env(agent, env)
            ...
            next_action = boltz_policy(next_state, q_table, temperature)
            # update q_table
