...
def qlearning(num_episodes = 500, gamma_discount = 0.95,  epsilon = 0.1):
    """
    Implementation of enhanced q-learning algorithm.
    ...
    for episode in range(0, num_episodes):
        ...
        exp = 0 # initialize exp degree to zero
        memo = list()
        iteration = 0
        while(game_end == False):
            ...
            q_th = max(q_table[:, state]) - min(q_table[:, state])
            delta_v = max(q_table[:, state]) - max(q_table[:, state_prev])
            ...
            # use temp to choose a suitable action with boltzmann policy
            action = boltz_policy(state, q_table, temp)
            ...
            alpha = min(a_c/visited[state], a_max)
            alphas.append(alpha)
            # store in M
        ...
        while(iteration>0):
            ...
            iteration = iteration - 1
        if(episode > 498):
            print("Agent trained with EQL after 500 iterations")
