...
def sa_q_learning(env, learn_st, test_st, learning=0.5, discount=0.99, back=False): 
    # Initialize Q table
    ...
    while reached < 300:
        ...
        while done != True:   
            # Select one action arbitrarily
            action_r = np.random.choice(env.action_space.n)
            # Another according to policy
            action = boltz_policy(state_adj[0], state_adj[1], Q, temp)

            eps = np.random.random()

            if (eps < math.exp((Q[state_adj[0], state_adj[1], action_r] -
                            Q[state_adj[0], state_adj[1], action]) / temp)):
                action = action_r
    ...
    tot_episodes = 0
    reached = 0
    # Run SA-Q learning algorithm: Testing Phase
    while reached < 40:
        # Initialize parameters
        done = False
        tot_reward, reward = 0,0
